      ...
      <div className="flex grow flex-row justify-start gap-1">
        {navData.length > 0 && (
          <div className="dropdown flex-none lg:hidden">
            ...
            <ul
              tabIndex={0}
              className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-neutral rounded-box"
            >
              {navData.map((item) => {
                return (
                  <li key={item.title}>
                    {item.target === null ? (
                      <p className="pointer-events-none text-nowrap">
                        {item.title}
                      </p>
                    ) : (
                      <Link href={item.target} className="text-nowrap">
                        {item.title}
                      </Link>
                    )}
                    {item.sub.length > 0 && (
                      <ul>
                        {item.sub.map((i) => (
                          <li key={i.title}>
                            <Link href={i.target} className="text-nowrap">
                              {i.title}
                            </Link>
                          </li>
                        ))}
